...
class PSH5Dataset(Data.Dataset):
    def __init__(self, h5file_path):
        data = h5py.File(h5file_path)  # NxCxHxW = 0x1x2x3=8806x8x64x64
        self.data=data
        # tensor type:
        gt1 = data["gt"][...]  # convert to np tpye for CV2.filter
        ...
        ms1_tmp = get_edge(ms1)  # NxHxWxC
        self.ms = torch.from_numpy(ms1_tmp).permute(0, 3, 1, 2)  # NxCxHxW:
        ...
        pan_hp_tmp = get_edge(pan1)  # NxHxW
        pan_hp_tmp = np.expand_dims(pan_hp_tmp, axis=3)  # NxHxWx1
        self.pan = torch.from_numpy(pan_hp_tmp).permute(0, 3, 1, 2)  # Nx1xHxW:
        print(
            f"gt: {self.gt.size()}, lms: {self.lms.size()}, pan: {self.pan.size()}, ms: {self.ms.size()} with {2047}")
    ...
    def __getitem__(self, index):
        ...
        '''return {'gt': self.gt[index, :, :, :].float(),
                ...
                'ms_hp': self.ms_hp[index, :, :, :].float(),
                'pan_hp': self.pan_hp[index, :, :, :].float()}'''
        '''return {'gt': torch.Tensor(self.gt[index, :, :, :].float()),
                        #'lms': self.lms[index, :, :, :].float(),
                'ms_hp': torch.Tensor(self.ms[index, :, :, :].float()),
                'pan_hp': torch.Tensor(self.pan[index, :, :, :].float())}'''
...
class PSH5Dataset(Data.Dataset):
    ...
    def __init__(self, h5file_path):
        ...
        print(f"lms: {self.lms.size()}, pan: {self.pan.size()}, ms: {self.ms.size()} with {2047}")
    ...
    def __getitem__(self, index):
        ...
        ms = self.ms[index, :, :, :].float()
        #####必要函数
        return torch.Tensor(ms), torch.Tensor(pan), torch.Tensor(gt),torch.Tensor(lms)
    def __len__(self):
